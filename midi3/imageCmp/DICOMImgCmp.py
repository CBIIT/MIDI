...
def process_batch(batch_dir1, batch_dir2, output_pdf):
    ...
    batch1_files = sorted([f for f in os.listdir(batch_dir1) if f.endswith('.dcm')])
    batch2_files = sorted([f for f in os.listdir(batch_dir2) if f.endswith('.dcm')])
...
def compare_dicom_image(file_path1, file_path2, output_pdf):
    ...
    try:
        ...
        print(f"SSIM is invalid (not a number) when comparing {file_path1} and {file_path2}, so skipping the comparison.")
    ...
    for sop_instance_uid in new_map:
        ...
        if id_old:
            ...
            if old_file_path:
                ...
                print(f"!!!!!!!not find match old_file_path for id_old ={id_old}")
        else:
