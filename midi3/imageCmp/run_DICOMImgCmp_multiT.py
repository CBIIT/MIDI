...
class MapProcessor(threading.Thread):
    def __init__(self, thread_id: int, sub_map: Dict[str, str], old_map, id_map, output_dir: str):
        ...
        self.id_map = id_map
        self.output_filename = Path(output_dir) / f"comparsion_result_{thread_id}.pdf"
    ...
    def run(self):
        ...
        print(f"Finished Thread-{self.thread_id} in {self.execution_time:.2f} seconds")
    ...
    def compare_dicom_image(self, file_path1, file_path2, output_pdf):
        ...
        self.display_image(ax[0], image1, cmap='gray', error_message=error_message1)

        ax[0].set_title('Image After MIDI')
        self.display_image(ax[1], image2, cmap='gray', error_message=error_message2)

        ax[1].set_title('Image Before MIDI')
        self.display_image(ax[2], mask_rgb, cmap='gray', error_message=error_message2)	   
        ...
        for sop_instance_uid in new_map:
            ...
            if id_new not in processed_uids:           
                ...
                if id_old:
                    ...
                    if old_file_path:
                        self.compare_dicom_image(new_file_path, old_file_path, str(output_pdf))
                    else:
                        missing_matching_curated_file += 1
                        print(f"!!!!!!!!error2--Not finding file path for ID_old :{id_old} -- in id map but does not exist.")
                else:
                    print(f"!!!!!!!!error1--Not finding matching ID_old for id_new:{id_new}, that is in {new_file_path}")
            else:
                print(f"Skipping processed UID: {id_new}")
            
            if counter_for_progress % 100 == 0:
                print(f"Thread-{thread_number}:{counter_for_progress}/{len(new_map)} processed")
        print(f"Thread-{thread_number}:Missing matching curated file count: {missing_matching_curated_file}")       
      
    def compare_images(self, image1, image2):
        win_size = min(image1.shape[0], image1.shape[1], image2.shape[0], image2.shape[1], 7) - 1
        ...
        ssim_index, diff = ssim(image1, image2, data_range=data_range,win_size=win_size, channel_axis=-1, full=True )
    ...
    def find_changed_area(self, image1, image2, threshold=0):
        ...
        if len(diff.shape) == 3:
            ...
            output_image = cv2.cvtColor(output_image.astype(np.uint8), cv2.COLOR_GRAY2BGR)
...
def build_dicom_map(folder_path, log_file):
    ...
    if log_file is None:
        ...
        log_file = os.path.join(os.getcwd(), f'dicom_processing_{timestamp}.log')
        ...
        for file in files:
            if file.endswith('.dcm'):
                ...
                if sop_instance_uid in dicom_map:
                    ...
                    print(f"Duplicate SOP Instance UID found: {sop_instance_uid}")
                    logger.info(f"Duplicate SOP Instance UID found: {sop_instance_uid}")
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['SOP instance uid', 'File path'])
        writer.writeheader()
        for sop_instance_uid, file_path in dicom_map.items():
            writer.writerow({'SOP instance uid': sop_instance_uid, 'File path': file_path})
...
def process_map_with_threads(new_map, old_map, id_map, output_dir, thread_number):
    ...
    with open(summary_file_path, "w") as summary_file:
        ...
        summary_file.write(f"Total execution time: {format_time(total_execution_time)}\n\n")
        
        # Write execution time for each thread
        summary_file.write("Thread Execution Times:\n")
        for thread in threads:
            summary_file.write(f"Thread-{thread.thread_id}: {format_time(thread.execution_time)}\n")

    print("\nExecution Summary:")
    print("-" * 50)
    print(f"Total execution time: {format_time(total_execution_time)}")
    print(f"Average time per thread: {format_time(total_execution_time/len(threads))}")
...
# Start the timer
    ...
    process_map_with_threads(new_map, old_map, id_map, output_pdf, thread_count)
